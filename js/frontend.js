...
function hottubPopup() {	
	...
	if ( jQuery('#TB_window').size() < 1 ) {
		var tbase = jht_ca_base();
		if ( tbase !== false ) {
			tb_show( null, tbase + '/wp-content/themes/jht/popup.html?TB_iframe=true&height=402&width=462',null);
		}
	}
	return false;
}
...
function hottubCookie() {
	if( (jQuery.cookie('hottubPopped') ) || ( jQuery('body').is('.brochure, .quote, .nopop' ) ) ) return false;
...
function jht_tpopit() {	
	...
	if ( jQuery('#TB_window').size() < 1 ) {
		var tbase = jht_ca_base();
		if ( tbase !== false ) {
			tb_show( null, tbase + '/dealer-locator/dealer_ip/get_dealer.php?popped=true&TB_iframe=true&height=229&width=531', null );
		}
	}
	return false;
}

function jht_ca_base() {
	var tbase = false;
	var tlogo = jQuery('.hd a.logo');
	if ( tlogo.size() > 0 ) {
		tbase = tlogo.attr('href');
	}
	
	if ( tbase ) {
		// actually for /fr/ we still want just CA version?
		tbase = tbase.replace( "/fr/", "/" );
	}
...
jQuery(function($) {
	...
	if(jQuery.cookie('hottubTPop')) {
		...
		if ( $('#tpop').size() > 0 ) { // dealer-loc already included inline on page
			...
			// otherwise, $.load it
			$('#fbar').after('<div style="display:none" id="salecatch" />').next().load('/dealer-locator/dealer_ip/get_dealer.php', function() {
				if ( $("#tpop").size() > 0 ) {
					jht_tpopit();
				}
			});
	...
	if ( $('body').hasClass('dir') ) {
		...
		if ( !$('body').hasClass('fr') ) hottubCookie();
	}
