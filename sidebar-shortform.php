...
<form action="http://<?php echo $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']; ?>" method="post" id="leadForm" class="pform bro">
    ...
    <table width="276">
        <tr>
            <td width="144"><?php avala_field('first_name', 'txt', true, NULL, array( 'size' => 14) ); ?></td>
            <td width="132"><?php avala_field('last_name', 'txt', true, NULL, array( 'size' => 15) ); ?></td>
        </tr>
        </td>
            <td colspan="2"><label for="person_zip"><?php if ( is_page( 'canada-brochure-request' ) ) { ?>Postal Code *<?php } else { ?>Zip *<?php } ?></label>
                <?php avala_field('postal_code', 'txt full', true, field, array( 'size' => 7) ); ?>
            </td>
        </tr>
        <tr>
            <td colspan="2"><?php avala_field('phone', 'txt full', true, NULL, array('size' => 12)); ?></td>
        </tr>
        <tr>
            <td colspan="2"><?php avala_field('email', 'txt full email', true, NULL, array('size' => 20)); ?></td>
        </tr>
    </table>
