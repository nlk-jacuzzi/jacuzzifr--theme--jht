...
/* - - - - - - - - - - JACUZZI SPECIFICS - - - - - - - - - - */
	...
	define( 'LEAD_SOURCE_ID', 15 );
	define( 'LEAD_SOURCE_ID_FR', 35 );
	define( 'LEAD_CATEGORY_ID', 9 );
	define( 'LEAD_TYPE_ID', 15 );
	define( 'LEAD_SOURCE_ID_PPC', 19 );
	define( 'LEAD_SOURCE_ID_FR_PPC', 36 );
	define( 'LEAD_SOURCE_ID_ORG', 18 );

	// Generic ID #s for Source, Category, Type
	define( 'LEAD_SOURCE_GENERIC', 8 );
	define( 'LEAD_CATEGORY_GENERIC', 4 );
	define( 'LEAD_TYPE_GENERIC', 13 );

	// More defines
	define( 'HOT_TUB_BRAND', 'Jacuzzi Hot Tubs' );
	...
	/* - - - - - Site Specific Functions - - - - - */
		...
		function avala_get_lead_source( $id = LEAD_SOURCE_GENERIC ) {
			$leadSource = array(
				1	=> 'Direct Mail',
				2	=> 'Sponsored Email',
				3	=> 'Promo',
				...
				10	=> 'Radio',
				11	=> 'Third Party',
				12	=> 'BuyerZone-Qualified',
				13	=> 'Buyerzone-Abandoned',
				14	=> 'Affiliate',
				15	=> 'JacuzziHotTubs.com',
				16	=> 'Co-Brand Out of Market',
				17	=> 'Historical',
				18	=> 'Search - Organic',
				19	=> 'Search - Paid',
				20	=> 'Consumer iPad',
				...
				24	=> 'Media-Videology',
				25	=> 'Media-AdRoll',
				26	=> 'Media-Rocket_Fuel',
				27	=> 'Media-PointRoll',
				28	=> 'Television',
				29	=> 'Billboard',
				30	=> 'BRC Card',
				31	=> 'Call Center',
				32	=> 'Referral and Rewards',
				33	=> 'Dealer Import',
				34	=> 'Rock-n-Roll',
				35	=> 'Jacuzzi.ca/fr',
				36	=> 'Jacuzzi.ca/fr - Paid',
				);
			if ( array_key_exists($id, $leadSource) )
				return apply_filters( 'avala_get_lead_source', $leadSource[$id] );
			return $leadSource[ LEAD_SOURCE_GENERIC ];

		}
		// Array of Lead Category ID's
		function avala_get_lead_category( $id = LEAD_CATEGORY_GENERIC ) {
			$leadCategory = array( 
		...
		function avala_get_lead_type( $id = LEAD_TYPE_GENERIC ) {
			$leadType = array( 
				1	=> 'Campaign',
				2	=> 'Contact Dealer',
				3	=> 'Request Brochure Mail',
				4	=> 'Request Financing',
				5	=> 'Request Quote',
				6	=> 'Request Test Drive',
				7	=> 'Request Trade In',
				8	=> 'Subscriber',
				9	=> 'Sweepstakes',
				10	=> 'Truck Load',
				...
				13	=> 'Other',
				14	=> 'Request DVD',
				15	=> 'Request Brochure Mail & DVD',
				16	=> 'Request Brochure Download & DVD',
				17	=> 'Request Brochure Mail & Download & DVD',
				18	=> 'Request Brochure Mail & Download',
				19	=> 'Dealer Entry',
				20	=> 'Request Buyer\'s Guide',
				);
...
function avala_form_submit( $redirect = true ) {
	...
	if ( isset($_POST['EmailAddress']) && !empty($_POST['EmailAddress']) && (strpos( $_POST['EmailAddress'], '@' ) !== false) && empty($_POST['Address']) )
	{
		...
		if ( $result[0] != 201 ) {
			mail( 'leaderror@ninthlink.com', 'JHT FORM ERROR :: ' . date( 'd/m/Y H:i:s', time() ), implode( " => ", $result ) . "\n\r\n\r" . $jsonArray['json_dump'] );
		}
...
function avala_hidden_fields( $leadSourceId = LEAD_SOURCE_ID, $leadCategoryId = LEAD_CATEGORY_ID, $leadTypeId = LEAD_TYPE_ID, $id = NULL, $campaignId = NULL ) {
	...
	if ( isset( $_GET['sa'] ) ) {
		if (strtolower($_GET['sa']) == 'l') 
		{
			//yes, it is ppc
			if ( $geo['country'] == 'FR' ) {
				$leadSourceId = LEAD_SOURCE_ID_FR_PPC;
			} else {
		...
		if (strtolower($_GET['sa']) == 't') 
		{
			//no, it is not ppc
			if ( $geo['country'] == 'FR' ) {
				$leadSourceId = LEAD_SOURCE_ID_FR_ORG;
			} else {
				$leadSourceId = LEAD_SOURCE_ID_ORG;
			}
		}
	}
	$leadSourceId 		= ( !empty($_GET['s_cid']) && $_GET['s_cid'] > 0) ? ( $geo['country'] == 'FR' ? LEAD_SOURCE_ID_FR_PPC : LEAD_SOURCE_ID_PPC ) : $leadSourceId;
	...
	echo '<input type="text" class="honey" name="Address" value="" placeholder="Leave this field blank, persons verification" data-role="none" />';
	// end honey bucket
...
function avala_get_thanks_page( $thanks_page = false ) {
	...
	$custom_progo = get_post_meta( $post->ID, '_progo' );
...
// lead tracking based on interest. if true, include tracking pixels in footer. see functions-trackingcode.php
...
function avala_args( $field = NULL, $content = NULL ) {
	...
	$pickyPostal = ( $geo['country'] == 'US' ) ? 'Zip Code' : 'Postal Code' ;
	...
	$args = array(
		...
		'product_use_fr'			=> array(
			'id'			=> 'custom_data_ProductUse',
			'classes'		=> '',
			'name'			=> 'CustomData[ProductUse]',
			'text'			=> 'Quel avantage vous attire le plus dans un spa?',
			'input_type'	=> 'radio',
			'options'		=> array(
				'Relaxation' => array( 'value' => 'Relaxation' ),
				'Soulagement des douleurs / Thérapie' => array( 'value' => 'Pain Relief/Therapy' ),
				'Liens avec parents et amis' => array( 'value' => 'Bonding/Family' ),
				'Organisation de fêtes dans la cour arrière' => array( 'value' => 'Backyard Entertaining' ),
				),
			'default_value'	=> '',
			),
		'condition'				=> array(
		...
		'product_id_list'		=> array(
			...
			'options'		=> array(
				...
				'J-415' => array( 'value' => '8' ),
				'J-375' => array( 'value' => '9' ),
				'J-365' => array( 'value' => '10' ),
				'J-355' => array( 'value' => '11' ),
				'J-345' => array( 'value' => '12' ),
				'J-335' => array( 'value' => '13' ),
				'J-325' => array( 'value' => '14' ),
				'J-315' => array( 'value' => '15' ),
				'J-280' => array( 'value' => '16' ),
				'J-275' => array( 'value' => '17' ),
				'J-245' => array( 'value' => '18' ),
				'J-235' => array( 'value' => '19' ),
				'J-210' => array( 'value' => '20' ),
				),
			'default_value'	=> '',
			),
		'newsletter'			=> array(
			...
			'text'			=> 'Please send me exclusive sale alerts from Jacuzzi Hot Tubs',
			'input_type'	=> 'checkbox',
			'default_value'	=> 'true',
			),
	);
	...
	{
		return apply_filters('avala_args', $args[$field]);
	}
	else
	{
		return $args;
